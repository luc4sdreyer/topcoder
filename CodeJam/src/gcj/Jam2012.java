...
public class Jam2012 {
	...
	private static String solve(int h, int w, int d, char[][] room) {
		int starty= 0;
		int startx=0;
		for (int j = 0; j< h; j++) {
			for (int k = 0; k< w; k++) {
				if (room[j][k] == 'X') {startx = j; starty = k; }
			}				
		}
	...
	static String filename = "D:\\gcj\\2012\\Q\\D-sample";
}